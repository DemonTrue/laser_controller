...
void HD44780_ClearArea(uint8_t col, uint8_t row, uint8_t length) {
    static char clear_buf[17]; // Буфер статический, чтобы не занимать стек. Макс 16 символов + \0

    if (length > 16) length = 16; // Защита от выхода за границы дисплея 16x2

    // Заполняем буфер пробелами
    for (uint8_t i = 0; i < length; i++) {
        clear_buf[i] = ' ';
    }
    clear_buf[length] = '\0'; // Нуль-терминатор

    HD44780_SetCursor(col, row);
    HD44780_PrintStr(clear_buf);
